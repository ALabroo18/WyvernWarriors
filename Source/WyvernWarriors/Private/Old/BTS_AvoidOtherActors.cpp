...
#include "Old/BTS_AvoidOtherActors.h"
#include "BehaviorTree/BlackboardComponent.h"
#include "AIController.h"
#include "Components/SphereComponent.h"

UBTS_AvoidOtherActors::UBTS_AvoidOtherActors()
{
	NodeName = "Avoid Other Actors";

	Interval = 0.1f; // Set the tick interval to 0.1 seconds
	RandomDeviation = 0.0f; // No random deviation
}

void UBTS_AvoidOtherActors::TickNode(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	FVector newLocation = FMath::VInterpTo(currentLocation, desiredLocation, OwnerComp.GetWorld()->GetDeltaSeconds(), 1.0f); // Interpolated new location based on average direction away from enemies
